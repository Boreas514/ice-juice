...
import zipfile
import threading
from io import StringIO # 3.0以后cStringIO被整合进io
import contextlib
import subprocess
import configparser # 3.0以后ConfigParser重命名为configparser
from time import sleep, strftime
from Crypto import Random # 该第三方库在win上需要VC++14.0进行编译
...
def show_aes():
    ...
    st_print('[*] Copy and add this key to another system running Stitch to enable communication from payloads created on this system.\n')
...
def find_path(text, line, begidx, endidx, dir_only=False, files_only=False, exe_only=False, py_only=False, uploads=False, all_dir=False):
    cur_dir = os.getcwd()
    before_arg = line.rfind(" ", 0, begidx)
    if before_arg == -1:
        return # arg not found
